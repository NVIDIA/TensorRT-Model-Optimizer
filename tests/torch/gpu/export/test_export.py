...
@pytest.mark.parametrize(
    ...
    [
        (  # Test replacements and KV cache scaling
            {
                "layer1.k_proj.output_quantizer._amax": torch.tensor([0.128]),
                "layer1.v_proj.output_quantizer._amax": torch.tensor([256.0]),
        ...
        (  # Test skipping output_quantizer _amax keys other than k_scale and v_scale
            {
                "layer1.q_proj.output_quantizer._amax": torch.tensor([0.128]),
                "layer1.k_proj.output_quantizer._amax": torch.tensor([0.128]),
                "layer1.v_proj.output_quantizer._amax": torch.tensor([256]),
            },
            KV_CACHE_FP8,
            128.0,
            {
