...
def _test_unified_import_megatron(tmp_path, rank, size):
    ...
    unified_export_megatron.import_mcore_gpt_from_hf(model, tiny_llama_dir)


def test_unified_import_megatron(tmp_path):
    spawn_multiprocess_job(
        size=1, job=partial(_test_unified_import_megatron, tmp_path), backend="nccl"
    )
