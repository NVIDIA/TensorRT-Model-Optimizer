...
def _test_speculative_gpt_model(
    ...
    attention_mask = torch.tril(
        torch.ones(
            (1, 1, max_sequence_length, max_sequence_length),
        ),
    ).cuda()
    ...
    if algo == "medusa":
        ...
        labels = torch.randint(
            0,
            vocab_size,
            (batch_size, max_sequence_length * (num_medusa_heads_or_eagle_layers + 1)),
        ).cuda()
        medusa_loss = model(prompt_tokens, position_ids, attention_mask, labels=labels)

        assert medusa_loss.shape[0] == batch_size
        assert medusa_loss.shape[1] == max_sequence_length * (num_medusa_heads_or_eagle_layers + 1)
    elif algo == "eagle":
        ...
        assert eagle_loss.shape[1] == max_sequence_length - 1
