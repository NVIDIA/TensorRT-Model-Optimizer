...
class DynamicCache(Cache):
    ...
    def from_batch_splits(cls, splits: list["DynamicCache"]) -> "DynamicCache":
        ...
        for idx in range(len(splits[0])):
            ...
            value_cache = [
                current.value_cache[idx] for current in splits if current.value_cache[idx].numel()
            ]
            if key_cache != []:
...
class HybridCache(Cache):
    ...
    def __init__(
        ...
        layer_switch = (
            config.sliding_window_pattern if hasattr(config, "sliding_window_pattern") else 2
        )  # 2 is for BC
...
class OffloadedStaticCache(StaticCache):
    ...
    def __init__(
        ...
        self.device = (
            torch.device(device) if layer_device_map is None else torch.device(layer_device_map[0])
        )
