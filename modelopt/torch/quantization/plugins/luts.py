...
from modelopt.torch.quantization.nn import TensorQuantizer, register_quant_backend, is_registered_quant_backend
...
class LutsScalarLut(torch.autograd.Function):
    ...
    def forward(ctx, inputs: torch.Tensor, amax: torch.Tensor | None, tq: TensorQuantizer):
        # TODO(Frank): Expose path to support other than the available formats.
        ...
        if encode_format not in _SCALAR_LUT_ENCODE_FORMATS:
            if encode_path is None:
                raise ValueError(f"Invalid encode format: {encode_format}. If path is not provided, a fixed encoding must be used.")
            else:
                if not os.path.isabs(encode_path):
                    raise ValueError(f"Invalid encode path: {encode_path}. It must be an absolute path.")
                if not os.path.exists(encode_path):
                    raise FileNotFoundError(f"luts encoder path {encode_path} does not exist.")
        ...
        values, bounds = luts.encode(encode_format, path=encode_path, norm=False, cuda=True)
...
def luts_entrypoint(inputs, tq: TensorQuantizer):
    ...
    if lut_type == "scalar_lut":
        ...
        raise NotImplementedError("Other lut types other than scalar_lut are not supported yet.")
