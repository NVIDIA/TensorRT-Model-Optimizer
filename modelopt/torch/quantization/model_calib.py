...
def _apply_weight_pre_quant_scale(linear, pre_quant_scale):
    if _ENABLE_FOLDING_PQS_TO_WEIGHTS:
        linear.weight.data.copy_(
            (linear.weight * pre_quant_scale.to(linear.weight.device).squeeze()[None, :]).to(linear.weight.dtype)
        )
    else:
...
def apply_pre_quant_scale_and_smooth(
    ...
    if linear.input_quantizer.amax is not None:
        ...
        linear.input_quantizer.amax = (_amax_for_smoothing * pre_quant_scale.to(device)).amax().to(dtype)
...
def awq_lite(
    ...
    **kwargs,
    ...
    def get_scale(x_max, w_max, alpha, tensor_parallel_group=None):
        scales = (
            (x_max.pow(alpha) / (w_max.to(x_max.device).pow(1 - alpha) + torch.finfo(torch.float32).tiny))
