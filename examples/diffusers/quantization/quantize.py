...
def create_pipeline(model_name: str, dtype: torch.dtype) -> DiffusionPipeline:
    """
    Create and return an appropriate pipeline based on the model_name provided.
    """
    if model_name == "sd3-medium":
        return StableDiffusion3Pipeline.from_pretrained(MODEL_ID[model_name], torch_dtype=dtype)
    elif model_name in ["flux-dev", "flux-schnell"]:
        return FluxPipeline.from_pretrained(MODEL_ID[model_name], torch_dtype=dtype)
    else:
        # Example for stable-diffusion-based models
        return DiffusionPipeline.from_pretrained(
            MODEL_ID[model_name],
            torch_dtype=torch.float16,  # Hard-coded to FP16 in your script
            variant="fp16",
...
def main() -> None:
    ...
    # Create pipeline
    pipe = create_pipeline(args.model, torch_dtype)
    pipe.to("cuda")
    ...
    if not args.restore_from:
        ...
        # Adjust the quant config with the desired TRT precision
    ...
    if args.onnx_dir:
        ...
        backbone.eval()
        with torch.no_grad():
            modelopt_export_sd(backbone, args.onnx_dir, args.model, args.format, args.model_dtype)
