...
if [ "$format" == "fp8" ]; then
    python quantize.py --model "$model" --format "$format" --batch-size 2 --calib-size 128 --percentile 1.0 --quant-level "$quant_level" --n_steps 20 --exp_name "$curt_exp" --collect-method "default"
else
    python quantize.py --model "$model" --format "$format" --batch-size 2 --calib-size 32 --percentile 1.0 --alpha 0.8 --quant-level "$quant_level" --n_steps 20 --exp_name "$curt_exp" --collect-method "min-mean"
fi
